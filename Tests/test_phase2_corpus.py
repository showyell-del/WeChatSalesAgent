...
from agent_core.corpus_builder import has_effective_interaction, static_exclusion
...
class FixtureClient(ChatlogClient):
    ...
    def history_page(self, chat, since, until, limit, offset, is_self):
        source = {
            False: [
                {"local_id": 1, "timestamp": 10, "sender": "客户", "type": "text", "content": "想体验街舞"},
                {"local_id": 2, "timestamp": 20, "sender": "客户", "type": "text", "content": "电话13800138000"},
                {"local_id": 3, "timestamp": 30, "sender": "客户", "type": "text", "content": "周六有时间"},
            ],
            True: [
                {"local_id": 4, "timestamp": 40, "sender": "店主", "type": "text", "content": "可以预约"},
            ],
        }[is_self]
...
class Phase2CorpusTests(unittest.TestCase):
    def test_directional_pagination_has_no_loss(self):
        ...
        self.assertEqual([item["is_self"] for item in messages], [False, False, False, True])

    def test_static_filters_and_bidirectional_requirement(self):
        self.assertEqual(static_exclusion({"username": "gh_demo", "chat": "公众号", "chat_type": "private", "is_group": 0}), "OFFICIAL_ACCOUNT")
        self.assertEqual(static_exclusion({"username": "123@chatroom", "chat": "群", "chat_type": "group", "is_group": 1}), "GROUP_CHAT")
        self.assertTrue(has_effective_interaction(
            [{"type": "text", "content": "想了解"}],
            [{"type": "text", "content": "您好"}],
        ))
        self.assertFalse(has_effective_interaction([{"type": "text", "content": "广告"}], []))

    def test_fact_extraction_keeps_evidence_binding(self):
        facts = extract_facts("孩子10岁，在工业园区，想周六体验街舞，预算1000元，电话13800138000", "ev_demo")
        fields = {item["field"] for item in facts}
        self.assertTrue({"age", "region", "available_time", "explicit_need", "budget_cny", "mobile"}.issubset(fields))
    ...
    def test_fact_extraction_rejects_generic_time_region_and_wechat_noise(self):
        facts = extract_facts("系统出款需要时间，所在城市投诉重灾区，到帐篷区的数据来自wxappTokenABC，这课程以后再看看", "ev_noise")
        fields = {item["field"] for item in facts}
    ...
    def test_evidence_id_changes_with_direction_or_content(self):
        base = {"local_id": 1, "timestamp": 10, "sender": "客户", "type": "text", "content": "你好", "is_self": False}
        first = evidence_id("account", "generation", "chat", base)
        changed = dict(base, is_self=True)
        self.assertNotEqual(first, evidence_id("account", "generation", "chat", changed))
    ...
    def test_corpus_conversation_schema_accepts_one_row(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            store.add_conversation(corpus, {"username": "chat", "chat": "客户"}, "eligible", "", 1, 1, 10)
