...
- 当前 DMG：`dist/WeChatSalesAgent-MVP-macOS-arm64.dmg`，SHA-256 `8810670842c6ae21ee58ecc0487a63502a4cb38c4d7c9703aee0503432593024`，133,869,724 bytes，生成于 2026-09-20 23:57:02 CST；133 项测试、Objective-C 编译、锁定依赖、无降级扫描、Phase 0 quick、Phase 4、只读挂载、`hdiutil verify`、签名、Chatlog 许可证、构建清单、MCP/WorkBuddy 连接器及安装文件一致性均通过。已覆盖安装并从 `/Applications/WeChatSalesAgent.app` 启动，安装版主程序 SHA-256 为 `02b4ca9b9c305c55bf01394fcfee61d31a7630469dae98ac6fd7df984e8a1153`，与只读挂载包逐字节一致。当前仍为 ad-hoc 签名，未做 Developer ID 公证。
- 2026-09-19：显式点名群聊的智能分析必须解析精确群名，在当前账号绑定的 Chatlog 中定位目标 `@chatroom`，读取指定时间窗内的全部可分析文本，再按消息/字符边界执行首轮分析和独立覆盖审计；不得回退到私聊 corpus 的宽关键词召回，也不得以候选上限截断。真实临时 DB 副本执行“帮我分析群聊：‘AI+🌞破壳HATCH’里讨论最热门的话题”已通过：只锁定 1 个群聊、读取 1,208 条文本，首轮 6 批+独立审计 6 批，约 56 秒返回 `agent.query.v2`、8 个话题分节和 4 个后续问题。未指定时间时内部仍可使用 36500 全历史值，但 UI 只能显示“全部历史”。App 必须安装标准“编辑”菜单以让文本控件通过第一响应者支持 Command-C/Command-V，并在任务提交时立即清空输入，不能在成功回调中删除用户的新草稿。智能分析 transcript 的用户问题独占上方右对齐段落且不显示“你”标签，Agent 内容在下方另起左对齐段落。本路径 116 项测试、Python/Node/Shell/Objective-C 检查、无兜底扫描和 2720×1640 原生过程态预览均通过。
- 2026-09-19：智能分析对话区的 `NSTextView` 文档宽度必须在 `NSScrollView` 每次布局时同步到可见内容宽度；只启用 `widthTracksTextView` 或 `NSViewWidthSizable` 会让文档宽于视口，导致右对齐用户消息实际落在屏幕外。已验证布局为：用户消息在上方最右侧的浅蓝圆角气泡中，Agent 回复在下一层左侧浅灰圆角卡片中，二者不在同一水平线；长 Agent 结果保持单一连续卡片。`--render-agent-preview` 与 `--render-agent-result-preview` 的 2720×1640 原生截图均已核验，116 项测试及 Phase 0/Phase 4 门禁通过。
- 2026-09-20：仪表盘、消息检索和智能分析必须按当前账号保留原生 root view，切页立即显示已有数据，再独立后台刷新；不得重建窗口、清空检索结果或用智能分析的串行占用标志阻塞只读刷新。账号重建时整体失效，异步回调同时校验 account ID、Chatlog 任务实例与 navigation revision。最终安装版实测：会话列表 1,444 条，杨凯时间窗消息匹配 1,473 条并显示 200 条，往返切页后结果和详情均保留；仪表盘返回后立即显示 858 条当日群消息、18 个活跃群聊和 31 个数据库，同时背景更新。
- 2026-09-20：安装版 App 打开后自动启动并持有 Chatlog `127.0.0.1:5030` 与本机 REST API `127.0.0.1:8765`，二者必须是 App 直接子进程；只允许接管同一 `agent_core.local_api serve`、同一状态库的遗留进程，其他 8765 占用必须明确失败。API 令牌保存在 Keychain service `com.wechat-sales-agent.local-api`，仅 health/OpenAPI 公开，业务端点必须 Bearer 鉴权并在每次读取或分析前校验 Chatlog DB 路径只属于 `active_account`。真实安装版验证了 health/status/会话/消息/仪表盘，并通过 `POST /v1/analyses` 提交之前失败的 `AI+🌞破壳HATCH` 任务：立即返回 202，operation 从 queued/running 进入 succeeded，最终返回 `topic_analysis`、8 个真实话题分节且无错误。
- 2026-09-20：安装版同时提供标准 MCP stdio 入口 `python3 -m agent_core.local_mcp`，作为本机 REST API 的薄适配层，固定暴露 `list_conversations`、`search_wechat_messages`、`get_dashboard`、`analyze_conversation`、`get_analysis_result` 五个工具；Bearer token 只从 Keychain 读取，不得写入 MCP 配置。WorkBuddy 5.5.6 已用安装包内 Python runtime 和 `Integrations/WorkBuddy/mcp.json` 实机启用该 MCP；真实任务完整传入“帮我分析群聊“AI+🌞破壳HATCH”里讨论最热门的话题。”后，operation `op_0a9d45eba22d4f1aba42d0c3f423790b` 从 running 进入 succeeded，WorkBuddy 显示 `topic_analysis`、1,208 条消息、119 位参与者并生成完整 HTML 报告。自动化输入 Unicode 必须使用可验证的完整文本粘贴；若只传入损坏的 `AI+HATCH`，应按原请求失败，禁止在 Agent 侧猜测群名、放宽严格 JSON 校验或增加兜底。
- 2026-09-20：群聊分析的会话列表和截止时间必须在任务开始时直接从当前账号绑定的 Chatlog 获取，禁止使用已发布 corpus 的 `sessions` 或 `until_ts` 截断新消息。实机重跑“AI+🌞破壳HATCH群聊最热门话题分析”后，从旧版的 1,208 条、截至 2026-09-18，更新为 1,295 条、截至 2026-09-20 17:53，两轮分析成功输出 8 个话题分节。WorkBuddy 最终回复只能显示用户要求的数据、结论、证据和必要局限；除非用户明确要求，不得展示 operation ID、工具调用、轮询、失败重试、调试过程、本机路径，也不得自动生成报告文件、Skill 或记忆。最终 WorkBuddy 回复仅显示数据基础和核心结论，未出现“索引延迟”、失败重试、operation ID、本机路径或“补充”段，工作空间也没有生成任何文件。WorkBuddy 连接器固定在分析前用 `list_conversations` 确认当前账号和目标会话，分析只提交一次，立即查询后按 10 秒间隔轮询，禁止 30–50 秒长等待和失败后重复发起。
