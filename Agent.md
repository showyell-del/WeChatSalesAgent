...
- The native AppKit workspace exposes the DeepSeek chain directly: save API Key to macOS Keychain through stdin, explicitly approve external data transfer, estimate candidate cost, run DeepSeek with a pre-send confirmation, and refresh the published result. In the current real runtime with no published analysis, it shows account `wxid_3prysbeqgvci22_9f8d` and 244 eligible private-chat customers rather than `未连接`; the July 28 preview confirmed all five controls fit the toolbar.
