...
def select_account_id(account_ids, requested=""):
    if requested:
        if requested not in account_ids:
            raise ChatlogError(
                "ACCOUNT_NOT_IN_CHATLOG_DB_PATHS",
                "Selected account was not present in Chatlog database paths.",
            )
        return requested
...
def command_health(args):
    ...
    emit(
        event(
            "chatlog_health",
            "failed",
            "CHATLOG_HEALTH_NOT_OK",
            "Chatlog health did not return ok.",
            {"status": str(status)},
        )
    )
    return 1
    ...
    if not account_ids:
        emit(
            event(
                ...
                "No xwechat_files account id was found in Chatlog database paths.",
            )
        )
        return 1
    ...
    if not accounts:
        emit(
            event(
                "account_list",
                "failed",
                "WECHAT_ACCOUNTS_EMPTY",
                "Chatlog has no running or historical accounts.",
            )
        )
        return 1
    for item in accounts:
        emit(
            event(
                "account_list",
                "passed",
                "WECHAT_ACCOUNT_AVAILABLE",
                "WeChat account is available.",
                {
    ...
    try:
        if not account_id:
            ...
            if len(account_ids) != 1:
                raise ChatlogError(
                    "ACCOUNT_SELECTION_REQUIRED",
                    "Select exactly one WeChat account before connecting.",
                )
            account_id = account_ids[0]
        store.upsert_account(account_id, "", "selected")
        ...
        emit(
            event(
                "account_connect",
                "passed",
                "ACCOUNT_KEY_AND_DATABASES_VERIFIED",
                "Account key and decrypted databases are ready.",
                {
                    "account_id": account_id,
    ...
    try:
        ...
        try:
            ...
            store.upsert_account(
                args.account_id, str(account.get("data_dir", "")), "selected"
            )
            store.set_active_account(args.account_id)
        finally:
            store.close()
        emit(
            event(
                ...
                "passed",
                "ACCOUNT_SWITCHED",
                "Historical WeChat account selected.",
                {
                    "account_id": args.account_id,
                },
            )
...
def command_sync(args):
    ...
    try:
        ...
        if not account_ids:
            raise ChatlogError(
                "ACCOUNT_PATH_NOT_FOUND",
                "No account id was found in Chatlog database paths.",
            )
        account_id = select_account_id(account_ids, args.account_id)
        ...
        emit(
            event(
                "database_key",
                "passed",
                "DATABASE_KEY_VERIFIED",
                "Database key and decrypted primary databases verified.",
                {
                    "account_id": account_id,
                    "verified_databases": str(len(verification["verified_databases"])),
                },
        ...
        sessions = client.all_sessions(page_size=args.limit)
        if not sessions:
            store.fail_generation(
                generation_id, "CHATLOG_SESSIONS_EMPTY", "Chatlog returned no sessions."
            )
            store.upsert_account(
                account_id,
                root,
                "failed",
                "CHATLOG_SESSIONS_EMPTY",
                "Chatlog returned no sessions.",
            )
            emit(
                event(
                    "sync",
        ...
        emit(
            event(
                ...
                "Fresh account-bound generation published.",
                {
                    "account_id": account_id,
                    "generation_id": generation_id,
        ...
        if generation_id:
            try:
                store.fail_generation(generation_id, code, message)
            except Exception as finalization_error:
                finalization_errors.append(f"generation: {finalization_error}")
        if account_id:
            try:
                store.upsert_account(account_id, root, "failed", code, message)
            except Exception as finalization_error:
                finalization_errors.append(f"account: {finalization_error}")
        if finalization_errors:
            code = "SYNC_STATE_FINALIZATION_FAILED"
            message = f"{message}; {'; '.join(finalization_errors)}"
        emit(event("sync", "failed", code, message))
        return 1
    finally:
        store.close()
