...
WECHAT_ID = re.compile(
    r"(?:微信(?:号)?|wx|vx|v信)\s*(?:号|是|[:：])\s*([A-Za-z][A-Za-z0-9_-]{5,19})|加我\s+([A-Za-z][A-Za-z0-9_-]{5,19})",
    re.IGNORECASE,
)
AGE = re.compile(r"(?<!\d)([1-9]|1[0-7])\s*岁")
GRADE = re.compile(
    r"(幼儿园|学前班|小学[一二三四五六]年级|[一二三四五六]年级|初[一二三]|高[一二三]|大[一二三四])"
)
MINOR_POSITIVE = re.compile(
    r"(孩子|儿童|少儿|幼儿|宝宝|小朋友|未成年|幼儿园|学前班|小学|初中|初[一二三]|高中|高[一二三]|(?<!\d)(?:[1-9]|1[0-7])\s*岁)"
)
MINOR_UNCERTAIN = re.compile(r"(学生|家长|上学|放学|寒假|暑假|年级)")
BUDGET = re.compile(
    r"(?:预算|价位|能接受|大概)\D{0,8}(\d{2,6})\s*(?:元|块)?|(?<!\d)(\d{2,6})\s*(?:元|块)(?!\d)"
)
KNOWN_REGION_NAMES = (
...
KNOWN_REGION = re.compile(
    r"((?:" + KNOWN_REGION_NAMES + r")(?:省|市|自治区|特别行政区|区|县|镇|街道)?)"
)
CONTEXT_REGION = re.compile(
    r"(?:住在|位于|地址(?:是|在)?|家在|店在|公司在|学校在|来自)\s*([\u4e00-\u9fff]{2,10}(?:省|市|区|县|镇|街道|小区))"
)
...
def minor_indicators(content: str) -> List[Dict[str, str]]:
    indicators = []
    for decision, pattern in (
        ("positive", MINOR_POSITIVE),
        ("uncertain", MINOR_UNCERTAIN),
    ):
        for match in pattern.finditer(content):
            item = {"decision": decision, "value": match.group(0)}
